...
namespace ECGViewer
{
	...
	public class ECGViewer : System.Windows.Forms.Form
	{
        ...
        private MenuItem menuExportMuseToTxt;
        private MenuItem menuExportMuseHeadToTxt;
		...
		private void InitializeComponent()
		{
            ...
            this.menuExportMuseToTxt = new System.Windows.Forms.MenuItem();
            ...
            this.menuExportMuseHeadToTxt = new System.Windows.Forms.MenuItem();
            ...
            this.menuExportMuseToTxt,
            this.menuExportMuseHeadToTxt});
            this.menuTools.Text = "Tools";
            // 
            // menuExportMuseToTxt
            // 
            this.menuExportMuseToTxt.Index = 0;
            this.menuExportMuseToTxt.Text = "ExportMuseDataToCSV";
            this.menuExportMuseToTxt.Click += new System.EventHandler(this.menuExportMuseToTxt_Click);
            // 
            // ECGPanel
            // 
            ...
            // 
            // menuExportMuseHeadToTxt
            // 
            this.menuExportMuseHeadToTxt.Index = 1;
            this.menuExportMuseHeadToTxt.Text = "ExportMuseHeadToTXT";
            this.menuExportMuseHeadToTxt.Click += new System.EventHandler(this.menuExportMuseHeadToTxt_Click);
            // 
            // ECGViewer
            // 
        ...
        void process_BackgroundWorkerCompleted(object sender, BackgroundWorkerEventArgs e)
        {
            ...
            {
                MessageBox.Show("Exceptions:" + e.BackGroundException.Message);
            }
        }
        private void menuExportMuseToTxt_Click(object sender, EventArgs e)
        {
            try
            {
                DialogResult dr = folderBrowserDialog.ShowDialog(this);
                ...
                if (dr == DialogResult.OK)
                {
                    ...
                    //process.BackgroundWork = this.exportMuseFiles;
                    //process.MessageInfo = "Exporting ";
                    ...
                    //DirectoryInfo theFolder = new DirectoryInfo(folderBrowserDialog.SelectedPath);

                    Thread exportMuseFileThread = new Thread(new ParameterizedThreadStart(exportMuseFiles));
                    exportMuseFileThread.IsBackground = true;
                    exportMuseFileThread.Start(folderBrowserDialog.SelectedPath);
        ...
        public static void exportMuseFiles(object data) 
        {
            String theSelectedPath = data as String;
            DirectoryInfo theFolder = new DirectoryInfo(theSelectedPath);
            FileInfo[] fileInfo = theFolder.GetFiles();
            int fileNumber = fileInfo.Length;
            int fileIndex = 0;
            //遍历文件
            foreach (FileInfo NextFile in fileInfo)  //遍历文件
            {
                fileIndex++;
                exportMuse(NextFile.FullName);
            }
            MessageBox.Show("ExportMuseFiles completed, Exported File Number: " + fileIndex.ToString());
        }

        private static bool exportMuse(string museFile) 
        {
            IECGFormat format = null;
            ...
            //    DialogResult dr = cfgScreen.ShowDialog(this);

            //    if (dr != DialogResult.OK)
            //        return false;
            //}

            format = reader.Read(museFile, 0, cfg);

            if (format != null)
            {
                IECGFormat tempECG = format;
                String newTextFile = museFile;
                String[] fileNames = newTextFile.Split('.');
                String[] filePaths = fileNames[0].Split('\\');

                StringBuilder FilePath = new StringBuilder("");
                for (int index = 0; index < filePaths.Length -1; index++) 
                {
                    FilePath.Append(filePaths[index]).Append("\\");
                }
                FilePath.Append("12LeadsExported\\");
                String tempFileName = filePaths[(filePaths.Length - 1)];
                String fileName = FilePath.Append(tempFileName).ToString();



                Signals tempSignal;

                if (tempECG.Signals.getSignals(out tempSignal) != 0)
                {
                    return false;
                }
                else 
                {
                    Signals sig = tempSignal.CalculateTwelveLeads();
                    return sig.SaveLeadsToFile(fileName + ".csv");
                }            
            }
            else
                return false;
        ...
        public static void exportMuseFileHeads(object data)
        {
            String theSelectedPath = data as String;
            DirectoryInfo theFolder = new DirectoryInfo(theSelectedPath);
            FileInfo[] fileInfo = theFolder.GetFiles();
            ...
            {
                fileIndex++;
                exportMuseHead(NextFile.FullName);
            }
            MessageBox.Show("ExportMuseFiles completed, Exported File Number: " + fileIndex.ToString());
        }
        private static bool exportMuseHead(string museFile)
        {
            IECGFormat format = null;
            ...
            {
                IECGFormat tempECG = format;
                String TopInfo = new String(' ', 1024);
                GetTopInfo(tempECG, out TopInfo);
                ...
                FilePath.Append("HeadInfoExported\\");
                String tempFileName = filePaths[(filePaths.Length - 1)];
                String fileName = FilePath.Append(tempFileName).ToString() + "_head.csv";
        ...
        public static bool GetTopInfo(IECGFormat format, out String TopInfo) 
        {
            if ((format == null)
            || (format.Demographics == null))
            {
            ...
            else
            {
                ...
                try
                {
                    ...
                    && (format.Demographics.FirstName.Length != 0))
                    {
                        sb2.Append(format.Demographics.FirstName);
                        sb2.Append(" ");
                    }

                    sb2.Append(format.Demographics.LastName);

                    if ((format.Demographics.SecondLastName != null)
                    && (format.Demographics.SecondLastName.Length != 0))
                    {
                        sb2.Append('-');
                        sb2.Append(format.Demographics.SecondLastName);
                    }

                    sb2.Append("@");


                    sb1.Append("Patient ID@");
                    sb2.Append(format.Demographics.PatientID);
                    sb2.Append("@");
                    ...
                    && (gms.measurment[0] != null))
                    {
                        ...
                        sb1.Append("Vent rate@");
                        PrintValue(sb2, ventRate, 3);
                        sb2.Append(" BPM@");

                        sb1.Append("PR int@");
                        PrintValue(sb2, PRint, 3);
                        sb2.Append(" ms@");


                        sb1.Append("QRS dur@");
                        PrintValue(sb2, QRSdur, 3);
                        sb2.Append(" ms@");
                        ...
                        PrintValue(sb2, QT, 3);
                        sb2.Append('/');
                        PrintValue(sb2, QTc, 3);
                        sb2.Append(" ms@");

                        sb1.Append("P-R-T axes@");
                        sb2.Append((gms.measurment[0].Paxis != GlobalMeasurement.NoAxisValue) ? gms.measurment[0].Paxis.ToString() : "999");
                        sb2.Append(' ');
                        sb2.Append((gms.measurment[0].QRSaxis != GlobalMeasurement.NoAxisValue) ? gms.measurment[0].QRSaxis.ToString() : "999");
                        sb2.Append(' ');
                        sb2.Append((gms.measurment[0].Taxis != GlobalMeasurement.NoAxisValue) ? gms.measurment[0].Taxis.ToString() : "999");
                        sb2.Append("@");
                    }
                    ...
                    if (birthDate != null)
                    {
                        sb2.Append(birthDate.Day.ToString("00"));
                        sb2.Append(birthDate.Month.ToString("00"));
                        sb2.Append(birthDate.Year.ToString("0000"));
                    ...
                    sb1.Append("Gen@");
                    if (format.Demographics.Gender != ECGConversion.ECGDemographics.Sex.Null)
                        sb2.Append(format.Demographics.Gender.ToString());
                    sb2.Append("@");

                    sb1.Append("Dep@");
                    sb2.Append(format.Demographics.AcqDepartment);
                    sb2.Append("@");


                    DateTime dt = format.Demographics.TimeAcquisition;

                    sb1.Append("TimeAcquisition@");
                    String TimeAcquisition = (dt.Year > 1000) ? dt.ToString("dd/MM/yyyy HH:mm:ss") : "Time Unknown";
                    sb2.Append(TimeAcquisition);
                    sb2.Append("@");
                    ...
                    && (format.Diagnostics.getDiagnosticStatements(out stat) == 0))
                    {
                        if (stat.statement != null)
                        {
                            ...
                            && !temp2.StartsWith("reviewed by", StringComparison.InvariantCultureIgnoreCase))
                            {
                                ...
                                //{
                                //    if (stat.confirmed)
                                //        sb2.Append("Confirmed by ");
                                //    else if (stat.interpreted)
                                //        sb2.Append("Interpreted by ");
                                //    else
                                //        sb2.Append("Reviewed by ");

                                //    sb2.Append(format.Demographics.OverreadingPhysician);

                                //}
                                //else
                                //    sb2.Append("UNCONFIRMED AUTOMATED INTERPRETATION");
                            }


                        }
                    }
                    TopInfo = sb1.ToString() + "\r\n" + sb2.ToString();
                }
                catch
                {
                    TopInfo = "";
        ...
        private void menuExportMuseHeadToTxt_Click(object sender, EventArgs e)
        {
            try
            {
                DialogResult dr = folderBrowserDialog.ShowDialog(this);
                ...
                if (dr == DialogResult.OK)
                {
                    ...
                    Thread exportMuseFileThread = new Thread(new ParameterizedThreadStart(exportMuseFileHeads));
                    exportMuseFileThread.IsBackground = true;
                    exportMuseFileThread.Start(folderBrowserDialog.SelectedPath);
